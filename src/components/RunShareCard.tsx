...
import { Clock, Download, Flame, Gauge, Image as ImageIcon, Layers3, Map, MapPin, Mountain, RefreshCw, Route as RouteIcon, Share2, ShieldAlert, ShieldCheck, Timer, Upload, X } from 'lucide-react';
...
function extractPoint(point: any): Point | null { if (!point) return null; const lat = Number(point.lat ?? point.latitude ?? point.location?.lat ?? point.location?.latitude); const lng = Number(point.lng ?? point.longitude ?? point.location?.lng ?? point.location?.longitude); return Number.isFinite(lat) && Number.isFinite(lng) ? { lat, lng } : null; }
function hasValidLatLng(point: any): boolean { return Boolean(extractPoint(point)); }
...
function normalizedRoute(points: Point[]) {
  ...
  const innerW = 760, innerH = 760;
  return points.map(p => ({ x: 120 + ((p.lng - minLng) / dx) * innerW, y: 120 + (1 - (p.lat - minLat) / dy) * innerH }));
}
...
export function RunShareCard({ session: rawSession, onClose }: RunShareCardProps) {
  ...
  const routeScaleAtPinchRef = useRef(1);
  ...
  const routePoints = useMemo(() => trajectory.map(extractPoint).filter((point): point is Point => Boolean(point)), [trajectory]);
  ...
  useEffect(() => {
    if (!hasRoute || compositionMode === 'photo-route') return undefined;
    ...
    const fetchMap = async (user: NonNullable<typeof auth.currentUser>) => {
      ...
      try {
        ...
        const response = await fetch(`${API_CONFIG.baseUrl}/api/activity-map`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${idToken}` }, body: JSON.stringify({ trajectory: routePoints, width: 720, height: 1280, mapType: mapVariant, zoomAdjust }) });
  ...
  }, [cacheKey, compositionMode, hasRoute, mapImages, mapVariant, routePoints, zoomAdjust]);

  const handlePhotoSelection = async (event: React.ChangeEvent<HTMLInputElement>, target: 'photo-map' | 'photo-route' = 'photo-map') => {
    ...
    try { setSelectedPhoto(await fileToDataUrl(file)); setCompositionMode(target); setCustomizerOpen(false); setFeedback(null); } catch { setFeedback('Não foi possível carregar essa foto.'); }
  };
  ...
  const onPointerDown = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (compositionMode === 'photo-route' && pointersRef.current.size === 1) dragRef.current = { id: event.pointerId, x: event.clientX, y: event.clientY, ox: routeOffset.x, oy: routeOffset.y };
    if (pointersRef.current.size === 2) { pinchDistanceRef.current = activePinch(); routeScaleAtPinchRef.current = routeScale; dragRef.current = null; }
  };
  const onPointerMove = (event: React.PointerEvent<HTMLDivElement>) => {
    if (!pointersRef.current.has(event.pointerId) || !hasRoute) return;
    pointersRef.current.set(event.pointerId, { x: event.clientX, y: event.clientY });
    if (compositionMode === 'photo-route' && pointersRef.current.size === 1 && dragRef.current?.id === event.pointerId) {
      setRouteOffset({ x: dragRef.current.ox + event.clientX - dragRef.current.x, y: dragRef.current.oy + event.clientY - dragRef.current.y });
      return;
    }
    if (pointersRef.current.size !== 2) return;
    const distance = activePinch(), base = pinchDistanceRef.current;
    if (!distance || !base) { pinchDistanceRef.current = distance; return; }
    if (compositionMode === 'photo-route') {
      setRouteScale(Math.max(.62, Math.min(1.9, routeScaleAtPinchRef.current * (distance / base))));
      return;
    }
  ...
  const endPointer = (event: React.PointerEvent<HTMLDivElement>) => { pointersRef.current.delete(event.pointerId); if (dragRef.current?.id === event.pointerId) dragRef.current = null; if (pointersRef.current.size < 2) pinchDistanceRef.current = null; };

  const routeOverlay = routeSvgPoints.length >= 2 ? <div style={{ position: 'absolute', zIndex: 1, top: '12%', right: '3%', width: '51%', height: '56%', transform: `translate(${routeOffset.x}px, ${routeOffset.y}px) scale(${routeScale})`, transformOrigin: 'center', filter: 'drop-shadow(0 0 9px rgba(255,166,0,.72))' }}>
    <svg viewBox="0 0 1000 1000" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
      ...
      <polyline points={routeSvgPoints.map(p => `${p.x},${p.y}`).join(' ')} fill="none" stroke="#ff9d00" strokeWidth="30" strokeLinecap="round" strokeLinejoin="round" opacity=".28" />
      <polyline points={routeSvgPoints.map(p => `${p.x},${p.y}`).join(' ')} fill="none" stroke="#ffc13b" strokeWidth="12" strokeLinecap="round" strokeLinejoin="round" filter="url(#routeGlow)" />
      {(() => { const start = routeSvgPoints[0], end = routeSvgPoints[routeSvgPoints.length - 1]; return <><g transform={`translate(${start.x} ${start.y})`}><circle r="34" fill="#fff"/><circle r="25" fill="#ffad12"/><path d="M-8 13V-15M-8-13h23l-7 8 7 8H-8" fill="#111" stroke="#111" strokeWidth="5" strokeLinejoin="round"/></g><g transform={`translate(${end.x} ${end.y})`}><circle r="31" fill="#fff"/><circle r="17" fill="#ffad12"/></g></>; })()}
    </svg>
  </div> : null;
  ...
  const rowStyle: React.CSSProperties = { display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: 7 };
  const optionStyle: React.CSSProperties = { minHeight: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, borderRadius: 13, border: '1px solid rgba(255,255,255,.12)', background: '#111316', color: '#f4f4f4', fontSize: 10, fontWeight: 800 };
  ...
  return createPortal(<div className="share-screen" role="dialog" aria-modal="true" aria-label="Compartilhar atividade">
    ...
    <div className="share-card-stage" style={{ touchAction: 'none' }} onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endPointer} onPointerCancel={endPointer}><div ref={cardRef} className={cn('share-card-art', `share-card-art--${compositionMode}`)}><div className="share-card-background" aria-hidden="true">{(compositionMode === 'photo-map' || compositionMode === 'photo-route') && selectedPhoto ? <img src={selectedPhoto} alt="" className="share-card-photo" /> : null}{compositionMode === 'map' && currentMapImage ? <img src={currentMapImage} alt="" className="share-card-map" /> : null}{compositionMode === 'photo-map' && hasRoute && currentMapImage ? <div className="share-card-route-overlay"><img src={currentMapImage} alt="" /></div> : null}{compositionMode === 'photo-route' ? routeOverlay : null}<div className="share-card-vignette" />{!currentMapImage && hasRoute && compositionMode === 'map' ? <div className="share-card-loading-map">{mapError ? 'MAPA INDISPONÍVEL' : 'CARREGANDO MAPA...'}</div> : null}{(compositionMode === 'photo-map' || compositionMode === 'photo-route') && !selectedPhoto ? <div className="share-card-photo-empty">SELECIONE UMA FOTO</div> : null}</div><div className="share-card-brand"><InvictusLogo size={64} /><div className="share-card-brand-copy"><strong>INVICTUS</strong><span>PERFORMANCE</span></div></div><div className="share-card-content">{statMarkup}</div></div></div>
    {customizerOpen ? <div style={sheetStyle} role="dialog" aria-label="Personalizar compartilhamento"><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}><div><div style={{ color: '#f6a916', fontSize: 10, fontWeight: 900, letterSpacing: '.16em' }}>PERSONALIZAR</div><strong style={{ fontSize: 16 }}>Escolha a composição</strong></div><button className="share-icon-button" style={{ width: 34, height: 34 }} onClick={() => setCustomizerOpen(false)} aria-label="Fechar opções"><X size={16} /></button></div><div style={rowStyle}><button style={{ ...optionStyle, borderColor: compositionMode === 'map' ? '#f6a916' : 'rgba(255,255,255,.12)' }} onClick={() => { if (hasRoute) { setCompositionMode('map'); setCustomizerOpen(false); } }} disabled={!hasRoute}><Map size={17} />Mapa</button>{selectedPhoto ? <button style={{ ...optionStyle, borderColor: compositionMode === 'photo-map' ? '#f6a916' : 'rgba(255,255,255,.12)' }} onClick={() => { setCompositionMode('photo-map'); setCustomizerOpen(false); }}><ImageIcon size={17} />Foto + mapa</button> : <label style={{ ...optionStyle, cursor: 'pointer' }}><Upload size={17} />Foto + mapa<input type="file" accept="image/*" className="sr-only" onChange={event => void handlePhotoSelection(event, 'photo-map')} /></label>}{selectedPhoto ? <button style={{ ...optionStyle, borderColor: compositionMode === 'photo-route' ? '#f6a916' : 'rgba(255,255,255,.12)' }} onClick={() => { setCompositionMode('photo-route'); setCustomizerOpen(false); }} disabled={!hasRoute}><RouteIcon size={17} />Foto + rota</button> : <label style={{ ...optionStyle, cursor: 'pointer', opacity: hasRoute ? 1 : .4 }}><Upload size={17} />Foto + rota<input type="file" accept="image/*" className="sr-only" disabled={!hasRoute} onChange={event => void handlePhotoSelection(event, 'photo-route')} /></label>}</div>{hasRoute && compositionMode !== 'photo-route' ? <><div style={{ margin: '14px 0 8px', color: '#9d9d9d', fontSize: 10, fontWeight: 800, letterSpacing: '.12em' }}>ESTILO DO MAPA</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 8 }}><button style={{ ...optionStyle, flexDirection: 'row', borderColor: mapVariant === 'satellite' ? '#f6a916' : 'rgba(255,255,255,.12)' }} onClick={() => setMapVariant('satellite')}><Map size={16} />Satélite</button><button style={{ ...optionStyle, flexDirection: 'row', borderColor: mapVariant === 'outdoors' ? '#f6a916' : 'rgba(255,255,255,.12)' }} onClick={() => setMapVariant('outdoors')}><Mountain size={16} />Terreno</button></div></> : null}{compositionMode === 'photo-route' ? <div style={{ marginTop: 12, color: '#aaa', fontSize: 11, lineHeight: 1.35 }}>Arraste a rota com um dedo. Use pinça para aumentar ou diminuir.</div> : null}</div> : null}
