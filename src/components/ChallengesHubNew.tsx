...
const missionFilter = (mission: Mission): Filter => {
  if (mission.type === 'workout_count' || mission.type === 'gym_checkins') return 'workout';
  if (mission.type === 'cardio_minutes') return 'cardio';
  if (mission.type === 'streak_days' || mission.type === 'total_days') return 'habits';
  return 'performance';
};
...
export function ChallengesHubNew({ onCardio, onHistory }: Props) {
  ...
  const iconFor = (mission: Mission) => mission.type === 'cardio_minutes' ? Footprints : mission.type === 'streak_days' ? Flame : mission.type === 'workout_count' ? Dumbbell : CalendarCheck;
  const renderReward = (mission: Mission) => <><span>{mission.rewardXP.toLocaleString('pt-BR')} XP</span><span>+ {mission.rewardCoins.toLocaleString('pt-BR')} Invictus Coins <Coins /></span></>;
  ...
  return createPortal(<main className="dc-screen"><div className="dc-page">
    ...
    <div className="dc-section-head"><h2>DESAFIOS DISPONÍVEIS</h2><button onClick={onHistory}><History /> HISTÓRICO</button></div>
    {error ? <p className="dc-error">{error}</p> : null}{loading ? <p className="dc-loading">Carregando desafios reais…</p> : <section className="dc-list">{filtered.map(mission => { const item = progressFor(mission, progress); const Icon = iconFor(mission); const percentage = mission.target > 0 ? Math.min(100, item.currentProgress / mission.target * 100) : 0; return <article key={mission.id}><span className="dc-list-icon"><Icon /></span><div><small>DESAFIO {mission.category.toUpperCase()}</small><h3>{mission.title}</h3><p>{mission.description}</p></div><div className="dc-list-reward"><small>RECOMPENSA</small>{renderReward(mission)}</div><div className="dc-ring" style={{ '--progress': `${percentage * 3.6}deg` } as React.CSSProperties}><span>{item.currentProgress}/{mission.target}</span></div>{item.completed && !item.claimed ? <button disabled={claiming === mission.id} onClick={() => claim(mission)}>{claiming === mission.id ? '…' : 'RESGATAR'}</button> : item.claimed ? <span className="dc-claimed"><Check />RESGATADO</span> : <ChevronRight />}</article>; })}{filtered.length === 0 ? <p className="dc-empty">Nenhum desafio publicado nesta categoria.</p> : null}</section>}
    ...
    <section className="dc-premium"><Trophy /><div><h2>DESAFIOS PREMIUM</h2><p>Desafios PRO não oferecem dinheiro: ampliam reconhecimento, XP e Invictus Coins.</p></div><button onClick={() => setFilter('performance')}>VER DESAFIOS PRO <ArrowRight /></button></section>
