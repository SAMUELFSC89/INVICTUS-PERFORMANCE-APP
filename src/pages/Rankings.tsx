...
export function Rankings() {
  ...
  return <section className="ranking-flow ranking-flow--overview">
    ...
    <article className="rank-summary-card"><div><small>SUA POSIÇÃO</small><b>{myRank ? `${myRank}º` : '—'}</b><span>de {athletes.length || 0}</span></div><span className="rank-summary-emblem"><img src="/ranking-emblem-reference.png" alt="Emblema de posição Invictus" /></span><div><small>SUA PONTUAÇÃO (IGA)</small><b className="rank-gold">{Number(myEntry.score || 0).toLocaleString('pt-BR')}</b><span>Atualizado às {formatUpdatedAt(ranking?.updatedAt)}</span></div></article>
