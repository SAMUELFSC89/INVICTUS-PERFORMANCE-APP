...
function TopAvatar({ entry, rank }: { entry: any; rank: number }) {
  ...
  return <span className={`rank-avatar ${frame ? `rank-avatar--${frame}` : ''}`}><img src={avatar(entry)} alt={`Foto de ${name(entry)}`} referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.src = '/capacete.webp'; }} />{frame && <img className="rank-avatar-frame" src={`/ranking-frame-${frame}-reference.png`} alt="" aria-hidden="true" />}</span>;
}
