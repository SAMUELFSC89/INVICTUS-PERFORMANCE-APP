...
function Header({ onBack, info = true }: { onBack?: () => void; info?: boolean }) {
  return <header className="mus-header">
    ...
    {info ? <button aria-label="Informações"><Info /></button> : <span />}
  </header>;
}
...
function AiFlow({ draft, setDraft, onBack, onGenerate }: any) {
  ...
  return <><Header onBack={onBack} /><Steps current={draft.step} mode="ai" /><section className="mus-flow mus-ai-flow">
    {draft.step === 1 ? <><h1>CRIAR COM INVICTUS IA</h1><p>Responda algumas perguntas para que a IA monte o treino ideal para você.</p><h2>1. QUAL É O SEU PRINCIPAL OBJETIVO?</h2><div className="mus-choice-grid">{[['massa','GANHAR MASSA MUSCULAR'],['forca','GANHAR FORÇA'],['gordura','REDUZIR GORDURA CORPORAL'],['condicionamento','MELHORAR CONDICIONAMENTO'],['definicao','MELHORAR DEFINIÇÃO MUSCULAR'],['retorno','RETOMAR OS TREINOS'],['saude','SAÚDE E QUALIDADE DE VIDA']].map(([value,label]) => <Choice selected={draft.primaryGoal === value} onClick={() => setDraft({...draft,primaryGoal:value})} icon={<Target />} title={label} />)}</div><h2>OBJETIVOS SECUNDÁRIOS (OPCIONAL)</h2><div className="mus-check-grid">{['Aumentar força','Melhorar resistência','Ganhar mobilidade','Melhorar postura'].map(item => <Choice selected={draft.secondaryGoals.includes(item)} onClick={() => selectArray('secondaryGoals',item,3)} title={item} />)}</div></> : null}
    {draft.step === 2 ? <><h1>2. QUAL É A SUA EXPERIÊNCIA?</h1><p>Seu nível define volume, intensidade e complexidade dos movimentos.</p><div className="mus-choice-grid is-three">{[['iniciante','INICIANTE','Nunca treinei ou estou começando'],['intermediario','INTERMEDIÁRIO','Treino com consistência'],['avancado','AVANÇADO','Tenho ampla experiência']].map(([value,title,detail]) => <Choice selected={draft.experienceLevel === value} onClick={() => setDraft({...draft,experienceLevel:value})} icon={<Dumbbell />} title={title} detail={detail} />)}</div><h2>TEMPO DE EXPERIÊNCIA</h2><div className="mus-choice-grid is-four">{['Menos de 6 meses','6 a 12 meses','1 a 3 anos','Mais de 3 anos'].map(item => <Choice selected={draft.experienceTime === item} onClick={() => setDraft({...draft,experienceTime:item})} title={item} />)}</div></> : null}
    {draft.step === 3 ? <><h1>3. QUAL É A SUA ROTINA ATUAL?</h1><p>Isso ajuda a IA a montar um plano realista para o seu dia a dia.</p><h2>QUANTOS DIAS POR SEMANA VOCÊ TREINA?</h2><div className="mus-choice-grid is-seven">{[1,2,3,4,5,6,7].map(n => <Choice selected={draft.daysPerWeek === n} onClick={() => setDraft({...draft,daysPerWeek:n})} title={`${n} ${n === 1 ? 'dia' : 'dias'}`} />)}</div><h2>QUANTO TEMPO POR TREINO?</h2><div className="mus-choice-grid is-four">{[[45,'Até 45 minutos'],[60,'45 a 60 minutos'],[90,'60 a 90 minutos'],[120,'Mais de 90 minutos']].map(([n,label]) => <Choice selected={draft.durationMinutes === n} onClick={() => setDraft({...draft,durationMinutes:n})} title={String(label)} />)}</div><h2>HORÁRIO PREFERIDO</h2><div className="mus-choice-grid is-three">{['Manhã','Tarde','Noite'].map(item => <Choice selected={draft.preferredPeriod === item} onClick={() => setDraft({...draft,preferredPeriod:item})} icon={<Clock3 />} title={item} />)}</div></> : null}
    {draft.step === 4 ? <><h1>4. QUAIS EQUIPAMENTOS VOCÊ TEM ACESSO?</h1><p>A IA limitará o plano aos equipamentos que você selecionar.</p><div className="mus-choice-grid is-four">{equipmentOptions.map(([value,label]) => <Choice selected={draft.equipment.includes(value)} onClick={() => selectArray('equipment',value)} icon={<Dumbbell />} title={label} />)}</div><h2>ACESSÓRIOS (OPCIONAL)</h2><div className="mus-check-grid">{['Cinto','Munhequeira','Joelheira','Straps'].map(item => <Choice selected={draft.accessories.includes(item)} onClick={() => selectArray('accessories',item)} title={item} />)}</div></> : null}
    {draft.step === 5 ? <><h1>5. QUAIS SÃO AS SUAS PREFERÊNCIAS?</h1><p>Esses detalhes ajudam a IA a personalizar ainda mais seu plano.</p><h2>TIPO DE TREINO PREFERIDO</h2><div className="mus-choice-grid is-three">{[['forca','TREINO DE FORÇA'],['funcional','TREINO FUNCIONAL'],['cardio','CONDICIONAMENTO E CARDIO']].map(([value,label]) => <Choice selected={draft.preferredTraining === value} onClick={() => setDraft({...draft,preferredTraining:value})} icon={<Dumbbell />} title={label} />)}</div><h2>DIVISÃO PREFERIDA</h2><div className="mus-choice-grid is-three">{['Full body','Upper / Lower','ABC (3x por semana)','Bro split','PPL','Outro'].map(item => <Choice selected={draft.preferredSplit === item} onClick={() => setDraft({...draft,preferredSplit:item})} title={item} />)}</div><h2>PREFERÊNCIAS ADICIONAIS</h2><div className="mus-check-grid">{['Prefiro treinos sem impacto','Quero queimar mais gordura','Tenho pouco tempo disponível','Quero treinos desafiadores'].map(item => <Choice selected={draft.preferences.includes(item)} onClick={() => selectArray('preferences',item)} title={item} />)}</div></> : null}
