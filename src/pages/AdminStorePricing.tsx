...
type ProductConfigurationDraft = Partial<PhysicalProduct> & { cashPrice?: number | null };
...
const pendingLabel: Record<string, string> = {
  ...
  CASH_PRICE: 'preço em dinheiro', COIN_DISCOUNT: 'preço com desconto + Coins', STOCK: 'estoque', LOGISTICS: 'peso e dimensões logísticas',
...
export function AdminStorePricing() {
  ...
  const [configuration, setConfiguration] = useState<ProductConfigurationDraft>({});
  ...
  useEffect(() => {
    ...
    setConfiguration({ productStatus: selected.productStatus, supplierId: selected.supplierId, supplierSku: selected.supplierSku, gtin: selected.gtin, currentSupplierCost: selected.currentSupplierCost, minimumOrderQuantity: selected.minimumOrderQuantity, leadTime: selected.leadTime, publicDescription: selected.publicDescription, publicHighlights: selected.publicHighlights, cashPrice: selected.pricing.cashPrice, coinDiscountPrice: selected.coinDiscountPrice, coinDiscountAmount: selected.coinDiscountAmount, commercialStock: selected.commercialStock, dropStock: selected.dropStock, displayOrder: selected.displayOrder, active: selected.active, storeVisible: selected.storeVisible, published: selected.published, canPurchaseWithCash: selected.canPurchaseWithCash, canPurchaseWithCoinsDiscount: selected.canPurchaseWithCoinsDiscount });
  }, [selected?.productId]);
  ...
  const configNumber = (key: keyof ProductConfigurationDraft) => configuration[key] == null ? '' : String(configuration[key]);
  const updateConfigNumber = (key: keyof ProductConfigurationDraft, raw: string) => setConfiguration(current => ({ ...current, [key]: raw === '' ? null : Number(raw) }));
  ...
  return <div className="admin-store">
    ...
    {loading ? <p>Carregando…</p> : <div className="admin-store-layout">
      ...
      {selected ? <section className="admin-store-editor">
        ...
        <div className="admin-store-results"><article><span>CUSTO TOTAL ESTIMADO</span><b>{price?.estimatedProfit != null && price?.cashPrice != null ? `R$ ${(price.cashPrice - price.estimatedProfit).toFixed(2)}` : '—'}</b></article><article><span>LUCRO ESTIMADO</span><b>{price?.estimatedProfit != null ? `R$ ${price.estimatedProfit.toFixed(2)}` : '—'}</b></article><article><span>MARGEM LÍQUIDA</span><b>{price?.estimatedMarginPercent != null ? `${price.estimatedMarginPercent.toFixed(2)}%` : '—'}</b></article><article><span>MARKUP</span><b>{price?.markup != null ? `${price.markup.toFixed(2)}x` : '—'}</b></article><article><span>PREÇO SUGERIDO</span><b>{price?.calculatedSuggestedPrice != null ? `R$ ${price.calculatedSuggestedPrice.toFixed(2)}` : '—'}</b></article><article><span>PREÇO COM DESCONTO</span><b>{selected.coinDiscountPrice === null ? 'NÃO DEFINIDO' : `R$ ${selected.coinDiscountPrice.toFixed(2)}`}</b></article><article><span>COINS DO DESCONTO</span><b>{selected.coinDiscountAmount === null ? 'NÃO DEFINIDO' : <><Coins />{selected.coinDiscountAmount}</>}</b></article></div>
        ...
        <div className="admin-store-fields"><label><span>Preço normal em dinheiro</span><div><input type="number" min="0" step="0.01" value={configNumber('cashPrice')} onChange={event => updateConfigNumber('cashPrice', event.target.value)} /><b>R$</b></div></label><label><span>Preço com desconto</span><div><input type="number" min="0" step="0.01" value={configNumber('coinDiscountPrice')} onChange={event => updateConfigNumber('coinDiscountPrice', event.target.value)} /><b>R$</b></div></label><label><span>Coins do desconto</span><div><input type="number" min="0" step="1" value={configNumber('coinDiscountAmount')} onChange={event => updateConfigNumber('coinDiscountAmount', event.target.value)} /><b>Coins</b></div></label><label><span>Estoque comercial</span><div><input type="number" min="0" step="1" value={configNumber('commercialStock')} onChange={event => updateConfigNumber('commercialStock', event.target.value)} /><b>un.</b></div></label><label><span>Estoque legado do Drop</span><div><input type="number" min="0" step="1" value={configNumber('dropStock')} onChange={event => updateConfigNumber('dropStock', event.target.value)} /><b>un.</b></div></label><label><span>Ordem da vitrine</span><div><input type="number" min="0" step="1" value={configNumber('displayOrder')} onChange={event => updateConfigNumber('displayOrder', event.target.value)} /><b>#</b></div></label></div>
        <div className="admin-store-toggles">{([['active', 'Produto ativo'], ['storeVisible', 'Visível na loja'], ['published', 'Publicado'], ['canPurchaseWithCash', 'Compra em dinheiro'], ['canPurchaseWithCoinsDiscount', 'Desconto com Coins']] as Array<[keyof PhysicalProduct, string]>).map(([key, label]) => <label key={key}><input type="checkbox" checked={Boolean(configuration[key])} onChange={event => setConfiguration(current => ({ ...current, [key]: event.target.checked }))} /><span>{label}</span></label>)}</div>
