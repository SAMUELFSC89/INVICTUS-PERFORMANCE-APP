...
export function HealthReport() {
  ...
  return <main className="health-screen"><div className="health-content health-report"><HealthHeader title="RELATÓRIO DE SAÚDE" subtitle="Análise completa do seu desempenho e evolução." onBack={() => navigate('/health')} right={<div className="health-report-actions"><button onClick={() => window.print()}><Download />EXPORTAR</button><button><SlidersHorizontal />FILTRAR</button></div>} /><PeriodControl value={range} onChange={setRange} compact /><section className="health-report-metrics"><MetricCard icon={<Flame />} label="CALORIAS" value={calories.toLocaleString('pt-BR')} unit="kcal" detail="Total do período" /><MetricCard icon={<Clock3 />} label="TEMPO ATIVO" value={formatDuration(active)} detail="Total do período" /><MetricCard icon={<Dumbbell />} label="TREINOS" value={workoutCount} detail="Sessões validadas" /><MetricCard icon={<Heart />} label="FC MÉDIA" value={heartRate || '—'} unit={heartRate ? 'bpm' : ''} detail="Dados de sensor" tone="red" /><MetricCard icon={<MapPin />} label="DISTÂNCIA" value="—" unit="km" detail="Sincronize GPS" /></section><section className="health-report-chart"><div><h2>TENDÊNCIA DE CALORIAS <Info /></h2><strong>{calories.toLocaleString('pt-BR')} <small>kcal</small></strong><p>Total no período</p></div><ChartBars points={points.length ? points : [{ label: '—', value: 0 }]} /></section><section className="health-dual health-report-dual"><ZoneChart state={state} /><article className="health-trend-card"><div className="health-section-name">FC MÉDIA (POR DIA) <Info /></div><strong>{heartRate || '—'} <small>bpm</small></strong><p>Média no período</p><ChartBars points={state.computedMetrics.avg_heart_rate?.historyPoints.map((item) => ({ label: item.date, value: item.value })) || [{ label: '—', value: 0 }]} color="violet" /><button className="health-card-link">VER DETALHES DE FC <ChevronRight /></button></article></section><section className="health-dual"><article className="health-activity-card"><div className="health-section-name">ATIVIDADE POR TIPO <Info /></div><div className="health-activity-total"><strong>{workoutCount}</strong><span>Treinos</span></div><button className="health-card-link">VER TODOS OS TREINOS <ChevronRight /></button></article><article className="health-weekly-card"><div className="health-section-name">RESUMO SEMANAL <Info /></div>{weeklyRows.length ? weeklyRows.map((workout) => <div key={workout.id}><span>{new Date(workout.timestamp).toLocaleDateString('pt-BR')}</span><b>{Math.round(workout.caloriesBurned || 0)} kcal</b><span>{formatDuration(workout.durationMinutes)}</span></div>) : <p className="health-empty">Nenhuma atividade no período.</p>}</article></section><section className="health-advanced"><div className="health-section-name">MÉTRICAS AVANÇADAS <Info /></div><div><span>VO₂ MÁX. ESTIMADO <b>—</b><small>Requer sensor</small></span><span>CARGA DE TREINO <b>{Math.round(active * 1.5)}</b><small>Volume x frequência</small></span><span>RECUPERAÇÃO <b>{metricNumber(state, 'recovery_index')}%</b><small>{state.readinessStatus}</small></span><span>ÍNDICE DE CONSISTÊNCIA <b>{Math.min(100, Math.round((workoutCount / 5) * 100))}%</b><small>Atividade no período</small></span></div></section></div></main>;
}
