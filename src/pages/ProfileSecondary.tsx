...
export function ProfileSecondary() {
  ...
  return <main className="profile-flow-screen"><header><button onClick={back} aria-label="Voltar"><ArrowLeft /></button><h1>{page === 'preferences' && section ? (documents[section]?.title || gameViewTitles[section] || (section === 'game' ? 'ENTENDA O JOGO' : title[page])) : title[page]}</h1></header>{notice && <p className="profile-flow-notice"><X />{notice}</p>}<div className="profile-flow-content">{content()}{page === 'preferences' && !section && (user as any)?.role === 'admin' && <button className="profile-flow-primary" onClick={()=>go('/admin')}>PAINEL ADMINISTRATIVO</button>}</div>{withdrawPresenceCheck && <VerifiedPresenceModal isOpen={!!withdrawPresenceCheck} presenceCheckId={withdrawPresenceCheck.presenceCheckId} livenessPrompt={withdrawPresenceCheck.livenessPrompt} userMessage={withdrawPresenceCheck.userMessage} onClose={()=>setWithdrawPresenceCheck(null)} onSuccess={handleWithdrawPresenceSuccess} />}</main>;
}
