...
import { AlertTriangle, ArrowLeft, Bell, Building2, Check, CheckSquare, ChevronRight, CircleDollarSign, FileText, Globe2, HelpCircle, Landmark, Languages, LockKeyhole, MapPin, Moon, Plus, RefreshCw, Search, Settings, ShieldCheck, Target, Trash2, Trophy, UserRound, Watch, WalletCards, Wifi, X } from 'lucide-react';
...
export function ProfileSecondary() {
  ...
  // Antes de mover dinheiro real (WithdrawalEngine), o saque exige confirmacao
  // de presenca por selfie -- mesmo mecanismo do check-in de academia e da
  // inscricao de campeonatos (ver api/_lib/presence-check-service.ts).
  const [withdrawPresenceCheck, setWithdrawPresenceCheck] = useState<{ presenceCheckId: string; livenessPrompt: string; userMessage: string } | null>(null);
  ...
  const requestWithdrawal = async (event: React.FormEvent) => { event.preventDefault(); if (!withdrawAmount || !pixKey) return; try { setLoading(true); const token = await auth.currentUser?.getIdToken(); const response = await fetch('/api/financial?action=withdraw', { method: 'POST', headers: { 'Content-Type': 'application/json', ...(token ? { Authorization: `Bearer ${token}` } : {}) }, body: JSON.stringify({ amount: Number(withdrawAmount), pixKey }) }); const data = await response.json(); if (!data.success) throw new Error(data.error || 'Solicitação não aprovada.'); if (data.presenceCheckRequired) { setWithdrawPresenceCheck({ presenceCheckId: data.presenceCheckId, livenessPrompt: data.livenessPrompt, userMessage: data.message }); return; } message(data.message || 'Saque solicitado com sucesso.'); await loadWallet(); } catch (e: any) { message(e.message || 'Não foi possível solicitar o saque.'); } finally { setLoading(false); } };
  // onSuccess do VerifiedPresenceModal: so entao o saque de fato acontece no
  // servidor (WithdrawalEngine, dentro de actionType 'withdrawal' em
  // api/_handlers/validate-presence.ts).
  const handleWithdrawPresenceSuccess = async (result: { status: string; userMessage: string; commitResult?: any }) => {
    setWithdrawPresenceCheck(null);
    setLoading(false);
    if (result.status === 'approved' && result.commitResult) {
      setWithdrawAmount(''); setPixKey('');
      message('Saque solicitado com sucesso. O valor foi retido em segurança durante a análise.');
      await loadWallet();
      return;
    }
    message(result.userMessage || 'Não foi possível confirmar sua presença com confiança suficiente para processar o saque. Tente novamente.');
  };
  ...
  const documents: Record<string, { title: string; content: string }> = { terms: { title: 'TERMOS DE USO', content: LEGAL_TERMS_OF_USE }, privacy: { title: 'POLÍTICA DE PRIVACIDADE', content: LEGAL_PRIVACY_POLICY }, health: { title: 'DADOS DE SAÚDE', content: LEGAL_HEALTH_DATA_POLICY }, antifraud: { title: 'TRANSPARÊNCIA E VALIDAÇÃO', content: LEGAL_ANTI_FRAUD_POLICY }, campaigns: { title: 'DESAFIOS, CAMPEONATOS E COINS', content: LEGAL_PROMOTIONAL_RULES }, deletion: { title: 'EXCLUSÃO DE CONTA E DADOS', content: LEGAL_ACCOUNT_DELETION_POLICY }, subscriptions: { title: 'ASSINATURAS E CANCELAMENTO', content: LEGAL_SUBSCRIPTIONS_POLICY }, disclaimers: { title: 'AVISOS LEGAIS', content: LEGAL_DISCLAIMERS }, consents: { title: 'CONSENTIMENTOS E PERMISSÕES', content: LEGAL_CONSENTS } };
  ...
  const faqCategoryOrder: FAQItem['category'][] = ['Geral', 'Conta & Perfil', 'Pontuação & IGA', 'Assinaturas PRO', 'Health & Wearables', 'Desafios e Power Lift', 'Campeonatos', 'Invictus Coins e Loja', 'Antifraude & Auditoria', 'Privacidade & LGPD'];
  ...
  </section>, rules: <section className="profile-flow-document"><h2>REGRAS DE RANKING E ATIVIDADES</h2><p>Somente atividades concluídas e validadas entram na classificação. Cada modalidade pode exigir GPS, sensores, presença ou vídeo.</p><h3>CAMPEONATOS</h3><p>O campeonato gratuito é social, sem prêmio em dinheiro e sem vínculo com academias. Campeonatos pagos de musculação e cardio permanecem EM BREVE e exigirão regulamento específico antes de qualquer cobrança.</p><h3>INTEGRIDADE</h3><p>Registros duplicados, inconsistentes ou manipulados podem ser desconsiderados, com revisão quando aplicável.</p></section>, ai: <section className="profile-flow-document"><h2>INVICTUS IA</h2><p>A análise automática auxilia recomendações, métricas e validação de registros. Ela pode encaminhar casos inconclusivos para revisão e não substitui profissional de saúde ou educação física.</p></section>, faq: faqView() };
  const gameViewTitles: Record<string, string> = { scoring: 'PONTUAÇÃO IGA', rules: 'REGRAS DE RANKING', ai: 'INVICTUS IA', faq: 'PERGUNTAS FREQUENTES' };
  ...
  return createPortal(<main className="profile-flow-screen profile-flow-screen-new"><header><button onClick={back} aria-label="Voltar"><ArrowLeft /></button><div><InvictusLogo size={40}/><span><b>INVICTUS</b><small>PERFORMANCE</small></span></div><span /></header><section className="profile-flow-heading"><small>PERFIL E CONTA</small><h1>{page === 'preferences' && section ? (documents[section]?.title || gameViewTitles[section] || (section === 'game' ? 'ENTENDA O JOGO' : title[page])) : title[page]}</h1></section>{notice && <p className="profile-flow-notice"><X />{notice}</p>}<div className="profile-flow-content">{content()}{page === 'preferences' && !section && (user as any)?.role === 'admin' && <button className="profile-flow-primary" onClick={()=>go('/admin')}>PAINEL ADMINISTRATIVO</button>}</div><nav className="profile-flow-footer"><button onClick={()=>go('/')}><InvictusLogo size={23}/><span>Início</span></button><button onClick={()=>go('/championships')}><Trophy/><span>Campeonatos</span></button><button className="is-plus" onClick={()=>go('/musculacao')}><Plus/></button><button onClick={()=>go('/challenges')}><ShieldCheck/><span>Desafios</span></button><button className="is-active" onClick={()=>go('/profile')}><UserRound/><span>Perfil</span></button></nav>{withdrawPresenceCheck && <VerifiedPresenceModal isOpen={!!withdrawPresenceCheck} presenceCheckId={withdrawPresenceCheck.presenceCheckId} livenessPrompt={withdrawPresenceCheck.livenessPrompt} userMessage={withdrawPresenceCheck.userMessage} onClose={()=>setWithdrawPresenceCheck(null)} onSuccess={handleWithdrawPresenceSuccess} />}</main>, document.body);
}
