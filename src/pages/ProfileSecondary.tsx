...
const title: Record<Page, string> = { academy: 'MINHA ACADEMIA', 'academy-search': 'ALTERAR ACADEMIA', 'academy-confirm': 'ACADEMIA SELECIONADA', wearables: 'DISPOSITIVOS E RELÓGIOS', goals: 'METAS', security: 'SEGURANÇA E PRIVACIDADE', preferences: 'CONFIGURAÇÕES' };
const pageFromPath = (path: string): Page => path.includes('academy/search') ? 'academy-search' : path.includes('academy/confirm') ? 'academy-confirm' : path.includes('academy') ? 'academy' : path.includes('wearables') ? 'wearables' : path.includes('goals') ? 'goals' : path.includes('security') ? 'security' : 'preferences';
...
export function ProfileSecondary() {
  ...
  useEffect(() => { if (page === 'wearables') void loadWearables(); }, [page, loadWearables]);
  ...
  </section>, rules: <section className="profile-flow-document"><h2>REGRAS DE RANKING E ATIVIDADES</h2><p>Somente atividades concluídas e validadas entram na classificação. Cada modalidade pode exigir GPS, sensores, presença ou vídeo.</p><h3>FREQUÊNCIA CARDÍACA</h3><p>Relógios, sensores e serviços conectados possuem limitações e podem entregar leituras diferentes, atrasadas ou incompletas. Quando a FC influenciar uma competição, o app apresenta um aceite específico antes da participação; ele não substitui a autorização para tratar dados de saúde.</p><h3>CAMPEONATOS</h3><p>O campeonato gratuito é social, sem prêmio em dinheiro e sem vínculo com academias. Campeonatos pagos de musculação e cardio ficam disponíveis quando há uma edição publicada, com regulamento específico exigido antes de qualquer cobrança.</p><h3>INTEGRIDADE E REVISÃO</h3><p>Registros duplicados, inconsistentes ou manipulados podem ser desconsiderados. A tela de detalhes da atividade permite contestar atividade, FC, amostras, validação, pontuação, classificação ou possível falha técnica.</p></section>, ai: <section className="profile-flow-document"><h2>INVICTUS IA</h2><p>A análise automática auxilia recomendações, métricas e validação de registros. Ela pode encaminhar casos inconclusivos para revisão e não substitui profissional de saúde ou educação física.</p></section>, faq: faqView() };
