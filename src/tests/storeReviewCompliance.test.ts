...
describe('store review compliance gates', () => {
  ...
  test('store-facing legal files do not advertise the retired paid Elite/PIX model', () => {
    ...
    expect(terms).toContain('permanecem **EM BREVE**');
  ...
  test('consumer profile does not expose payout entry to normal users', () => {
    const profile = read('src/pages/ProfileNew.tsx');
    expect(profile).toContain("const isAdmin =");
    expect(profile).toContain("isAdmin ? <button onClick={() => navigate('/profile/wallet')}");
    expect(profile).toContain('Prêmios e saques (Admin)');
  });
