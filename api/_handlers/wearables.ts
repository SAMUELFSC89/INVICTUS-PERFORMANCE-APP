...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const [configSnap, stravaSnap] = await Promise.all([
      configRef.get(),
      db.collection('strava_connections').doc(auth.uid).get()
    ]);
  ...
  const stravaConnected = stravaSnap.exists;
  const now = new Date().toISOString();
  ...
  // O perfil pode refletir a conexão para a interface. Campos de pontuação,
  // ranking e running_stats não são aceitos nem alterados nesta rota.
