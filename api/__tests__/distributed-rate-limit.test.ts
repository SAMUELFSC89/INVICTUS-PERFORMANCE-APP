...
describe('distributed API rate limiter', () => {
  ...
  test('uses an opaque document id and never persists the raw subject', async () => {
    const subjectId = 'firebase-uid-sensitive-123';
    await expect(consumeDistributedRateLimit({
      scope: 'purchase', subjectId, windowMs: 60_000, maxRequests: 2,
    })).resolves.toMatchObject({ allowed: true });

    expect(mockDb.store.size).toBe(1);
    const [[key, stored]] = Array.from(mockDb.store.entries()) as Array<[string, Record<string, any>]>;
    expect(key).toMatch(/^api_rate_limits\/api_[a-f0-9]{64}$/);
    expect(key).not.toContain(subjectId);
