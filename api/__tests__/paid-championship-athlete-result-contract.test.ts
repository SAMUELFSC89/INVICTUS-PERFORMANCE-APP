...
describe('paid championship athlete result contract', () => {
  test('authenticated progress exposes only the athlete result from the frozen settlement', () => {
    const handler = read('api/_handlers/championships.ts');
    const types = read('src/types/championships.ts');

    expect(handler).toContain("db.collection('championship_settlements').doc(championshipId).get()");
