...
/**
 * Cria checkout Asaas avulso para a taxa de inscrição.
 * Gate 1: a criação externa é protegida por lease transacional no documento
 * determinístico da inscrição. Duas selfies/requisições concorrentes nunca
 * criam dois checkouts; falha ambígua vira UNCERTAIN e exige reconciliação em
 * vez de tentar de novo e potencialmente duplicar cobrança.
 */
export async function criarInscricaoChampionship(
  ...
  const perfilSnap = await getActiveProfile(userId);
  ...
  const reservation = await db.runTransaction(async (transaction: any) => {
    ...
    if (current.status === 'paga' && current.paymentStatus === 'PAID') {
      throw new Error('Voce ja esta inscrito neste campeonato.');
    }
    if (current.status === 'pendente' && current.asaasCheckoutId) {
      return { existing: true, data: current };
    }
    ...
    if (current.checkoutCreationStatus === 'CREATING' && leaseUntil > now) {
      throw new Error('O checkout ja esta sendo criado. Aguarde a conclusao da solicitacao atual.');
    }
  ...
  if (reservation.existing) {
    const dados: any = reservation.data;
    return {
      championshipId,
      valor: dados.valor,
      jaExistia: true,
...
export async function confirmarInscricaoChampionshipPorCheckout(asaasCheckoutId: string) {
  ...
  const dados: any = doc.data();
  if (!await getActiveProfile(String(dados.userId || ''))) {
    return markInactivePaymentForReconciliation(doc, asaasCheckoutId);
  }
...
export async function confirmarInscricaoChampionshipPorPagamento(asaasPaymentId: string, valorPago?: number, asaasCheckoutId?: string) {
  ...
  const dados: any = doc.data();
  if (!await getActiveProfile(String(dados.userId || ''))) {
    return markInactivePaymentForReconciliation(doc, asaasPaymentId || asaasCheckoutId || '');
  }
  if (dados.status === 'paga' && dados.paymentStatus === 'PAID') {
    return { encontrada: true, jaEstavaPaga: true, userId: dados.userId, championshipId: dados.championshipId };
  }
