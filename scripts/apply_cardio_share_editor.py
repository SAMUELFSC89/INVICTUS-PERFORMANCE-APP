...
# 1) Pós-cardio: abrir o editor diretamente, sem reapresentar a tela de detalhe.
challenges_path = Path("src/pages/Challenges.tsx")
challenges = challenges_path.read_text()

pattern = re.compile(r"(?m)^(\s*)setFinishedActivityItem\(finishedItem\);$")
matches = list(pattern.finditer(challenges))
if len(matches) != 2:
    raise SystemExit(f"Challenges direct share: expected 2 setFinishedActivityItem matches, found {len(matches)}")

def direct_share(match: re.Match[str]) -> str:
    indent = match.group(1)
    return (
        f"{indent}setFinishedActivityItem(finishedItem);\n"
        f"{indent}if (sessionType === 'cardio') {{\n"
        f"{indent}  setShareCardData(buildShareableFromItem(finishedItem));\n"
        f"{indent}}}"
    )

challenges = pattern.sub(direct_share, challenges)
...
challenges = replace_once(
    ...
    "Challenges legacy cardio detail suppression",
)

challenges_path.write_text(challenges)


# 2) Editor gestual: frases independentes + pinça/drag no bloco de marca e métricas.
card_path = Path("src/components/RunShareCard.tsx")
card = card_path.read_text()

card = replace_once(card, "  Share2,\n  X,", "  Share2,\n  Trash2,\n  X,", "Trash2 import")

old_types = """type LayerTransform = { x: number; y: number; scale: number; rotation: number };
...
new_types = """type LayerTransform = { x: number; y: number; scale: number; rotation: number };
type PointerPosition = { x: number; y: number };
...
card = replace_once(card, old_types, new_types, "phrase types")
card = replace_once(card, '          stroke="#ff9d00"', '          stroke="#f3b324"', "Invictus route gold")
...
persistence_marker = "  const currentMapImage = mapImages[mapCacheKey] ?? null;\n\n  useEffect(() => {\n    if (!hasRoute"
persistence = """  const currentMapImage = mapImages[mapCacheKey] ?? null;
  ...
  useEffect(() => {
    if (!hasRoute"""
card = replace_once(card, persistence_marker, persistence, "preference persistence")
...
handlers = r'''  const updatePhrase = useCallback((id: PhraseId, updater: (phrase: PhraseLayer) => PhraseLayer) => {
...
'''
card = replace_once(card, insert_before, handlers + insert_before, "phrase handlers")
...
card = replace_once(card, info_block, phrase_render, "phrase rendering")
...
card = replace_once(
    ...
    "restore phrases control",
)
card_path.write_text(card)


# 3) CSS dos elementos gestuais.
...
.share-card-phrase--movement { top: 5.4%; left: 5.3%; text-align: left; }
.share-card-phrase--freedom { top: 5.4%; right: 5.3%; text-align: right; }
.share-card-phrase--choice { left: 5.3%; bottom: 5.2%; text-align: left; }
...
'''
css = replace_once(css, marker, marker + phrase_css, "phrase CSS")
css = replace_once(
    css,
...
# 4) Contrato de regressão do novo fluxo.
